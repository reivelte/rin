...
namespace argparse {
    ...
    template<typename T> std::string toString(const T &v) {
        ...
        // modified for suzuri/rin to compile with clang:
        // this compiles with GCC, but it looks like wstring_convert will be deprecated in C++26
        // so just remove it for now. the unicode problem is a problem for another day.
...
//         } else if constexpr (has_ostream_operator<T>::value) {
    ...
    template<> inline short get(const std::string &v) { return static_cast<short>(std::stoi(v)); }
    ...
    template<> inline unsigned int get(const std::string &v) { return static_cast<unsigned int>(std::stoul(v)); }
    template<> inline unsigned short get(const std::string &v) { return static_cast<unsigned short>(std::stoul(v)); }
    ...
    public:
        ...
        virtual void help() {
            ...
            if (has_options()) cout << endl << bold("Options:") << endl;
            ...
            cout << endl << endl << bold("Subcommands: ") << endl;
            for (const auto &[subcommand, subentry] : subcommand_entries) {
                cout << setw(17) << subcommand << " : ";
                subentry->subargs->welcome();
                cout << '\n';
            }
        }
